...
function DashboardTest() {
    ...
    return (
        ...
        {listOfNotes.map((note) => (<TakeNote3 className='TakeNote3Class' notes={note}/>))}
            ...
            <Grid container style={{ flexGrow: 1, width: '85vw', height: 'auto', position: 'relative', top:30, left: 340 }} spacing={2} columns={{ xs: 8, sm: 12, md: 12 }} >
                {listOfNotes.map((note) => (<Grid item lg={9} >
                    <TakeNoteThreeTest style={{ border: '1px solid red' }} notes={note} />
                </Grid>))}
            </Grid>
        </Box>
    )
